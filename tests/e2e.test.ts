...
describe('Trakt.tv AlloCiné Userscript E2E Tests', () => {
  ...
  async function testTraktPage(url: string, screenshotName: string) {
    ...
    try {
      ...
      // Auto-accept any cookie consent banners or popups BEFORE waiting to ensure they don't block visibility
      try {
      ...
      // Wait until rating badge is rendered with actual content (not loading)
      await page.waitForFunction(() => {
        const badge = document.querySelector('#allocine-trakt-rating-badge');
        if (!badge) return false;
        const text = badge.textContent || '';
        return text.includes('Allociné') && !text.includes('Loading...');
      }, { timeout: 30000 });
