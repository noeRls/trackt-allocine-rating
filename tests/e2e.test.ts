...
describe('Trakt.tv AlloCiné Userscript E2E Tests', () => {
  ...
  async function testTraktPage(url: string, screenshotName: string) {
    ...
    try {
      // Changed to 'load' as domcontentloaded can be flaky with userscripts firing late,
      // and extended timeout to handle trakt's sometimes slow load times.
      ...
      // Some elements can take a long time to load, so wait until badge is injected
      // and not showing "Loading..." or give up after a generous timeout
